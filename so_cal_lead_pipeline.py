...
def run_pipeline():
    ...
    for address, prop in grouped_properties.items():
        ...
        # -----------------------------------------------------------------
        # ROBUST KV CACHE MATCHING BY CITATION ID & NORMALIZED ADDRESS
        # -----------------------------------------------------------------
        cached_lead = None
        clean_target_addr = re.sub(r"[^\w\s]", "", address).strip().upper()

        if case_no in kv_cache:
            cached_lead = kv_cache[case_no]
        elif address in kv_cache:
            cached_lead = kv_cache[address]
        else:
            for k, v in kv_cache.items():
                if isinstance(v, dict):
                    v_addr = v.get("address", "").upper()
                    clean_v_addr = re.sub(r"[^\w\s]", "", v_addr).strip()
                    if clean_target_addr and (clean_target_addr in clean_v_addr or clean_v_addr.startswith(clean_target_addr)):
                        cached_lead = v
                        break

        if cached_lead and cached_lead.get("phone") and cached_lead.get("phone") not in ["PENDING UNMASK", "Unmasked Upon Purchase"]:
            print(f"[Credit Guardrail] Reusing cached phone ({cached_lead['phone']}) for {address} ($0 Tracerfy Credits)")
            trace_data = {
